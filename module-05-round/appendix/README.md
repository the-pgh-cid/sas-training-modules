# Module 05 ROUND appendix companions
...
```bash
python /absolute/path/to/repo/module-05-round/appendix/reference.py
python /absolute/path/to/repo/module-05-round/appendix/test_reference.py
```
...
```bash
Rscript /absolute/path/to/repo/module-05-round/appendix/reference.R
Rscript /absolute/path/to/repo/module-05-round/appendix/test_reference.R
```
...
```sas
%let APPENDIX_DIR=/absolute/path/to/repo/module-05-round/appendix;
...
Columns: `case_id,row_id,value,expected_rounded`.
...
The added probes surround integers -2, 0 and 2. They contain no halfway values. Matching these checks does not extend the original claim to exact or near ties, other rounding units, missing values or all floating-point behavior. The original five fixture values and trial scores are unchanged.

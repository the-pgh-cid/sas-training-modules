...
%macro values_check(case=, name=);
  ...
  data _null_;
    ...
    retain bad 0 n 0;
    n + 1;
  if average ne expected_average then bad + 1;
    if _is_last then do;
...
data _null_;
  merge contract_output(in=has_output)
        contract_before(in=has_before keep=row_id case_id _seq x y z rename=(row_id=before_id case_id=before_case _seq=before_seq x=before_x y=before_y z=before_z)) end=_is_last;
  retain bad 0 n 0;
  n + 1;
  ...
  if x ne before_x then bad + 1;
  if y ne before_y then bad + 1;
  if z ne before_z then bad + 1;

  if vtype(average) ne 'N' then bad + 1;
  if average ne expected_average then bad + 1;
  if _is_last then do;
    if n ne &expected_rows or n = 0 then bad + 1;

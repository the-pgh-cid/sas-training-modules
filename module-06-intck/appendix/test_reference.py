...
spec = importlib.util.spec_from_file_location("module06_reference", HERE / "reference.py")
...
class ReferenceTests(unittest.TestCase):
    ...
    def test_T3_preservation_types_and_order(self):
        ...
        self.check_expected(result, rows)
        pd.testing.assert_frame_equal(result[["case_id", "row_id"]], before[["case_id", "row_id"]])
        english_months = list(reference.MONTHS)
        for col in ["start_date", "end_date"]:
            self.assertTrue(is_datetime64_any_dtype(result[col]))
            restored = [f"{d.day:02}{english_months[d.month-1]}{d.year:04}" for d in result[col]]
            self.assertEqual(restored, before[col].tolist())
        for col in reference.RESULT_COLUMNS:
            self.assertTrue(is_numeric_dtype(result[col]), col)

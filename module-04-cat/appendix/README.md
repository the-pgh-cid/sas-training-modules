# Module 04 CAT: executable appendix
...
- `fixtures.csv`: one shared, fully quoted UTF-8 CSV for all languages. Columns: `case_id`, `row_id`, `first`, `middle`, `last`, `expected_first`, `expected_middle`, `expected_last`, `expected_name1`, `expected_name2`.
...
```bash
python /absolute/path/to/repo/module-04-cat/appendix/reference.py
python /absolute/path/to/repo/module-04-cat/appendix/test_reference.py
Rscript /absolute/path/to/repo/module-04-cat/appendix/reference.R
Rscript /absolute/path/to/repo/module-04-cat/appendix/test_reference.R
```

For SAS, submit the following two statements in an installed SAS session, with the absolute appendix path on that host:

```sas
%let APPENDIX_DIR=/absolute/path/to/repo/module-04-cat/appendix;
